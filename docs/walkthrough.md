...
### Phase 4.89: Fix Text Nút Batch Flash Bị Cắt Chữ + Bug Kẹt Mode + Bảng Batch Log Trùng Cột Số Thứ Tự

User chạy thử tính năng Batch Flash (Phase 4.88) trên app thật, gửi 2 screenshot ở 2 lượt phản hồi riêng: (1) nút chính hiển thị "tart Batc" thay vì "Start Batch" — chữ bị cắt vì nút quá hẹp so với text dài, yêu cầu rút gọn "chỉ cần hiển thị Start/Next là đủ rồi"; (2) bảng Batch Log có cột "#" tự thêm bị trùng với cột số thứ tự có sẵn của `QTableWidget` (số xanh bên trái), và các cột còn lại không lấp hết chiều rộng bảng, để trống bên phải — yêu cầu "căn lại các cột còn lại cho đều".

### Thay đổi

- **`gui/batch_flash.py`**: đổi toàn bộ text nút `flashButton` ở chế độ Batch từ `"Start Batch"` → `"Start"` và `"Next ECU"` → `"Next"` (giữ nguyên `"Abort"` — không bị cắt). Cập nhật đồng bộ theo các label/caption/message có nhắc tên nút này (`labelBatchStatus`, `labelBatchStatusCaption`, hộp thoại "No Firmware Loaded") để không bị lệch text so với nút thật.
- **`gui/menu_bar.py`** — bug thật phát hiện thêm trong lúc verify fix trên: `setup_settings_profile()` (gọi `load_profile()`, set `actionModeBatchFlash.setChecked(...)` từ `flash/mode` đã lưu) chạy **trước** `setup_menu_bar()` (nơi dựng `QActionGroup` cho 2 action Mode) — đúng thứ tự đã có từ Phase 4.88, có lý do riêng (menu bar cần `self._settings` đã sẵn sàng để dựng Recent Files). Hậu quả: nếu `flash/mode` đã lưu là `"batch"`, `load_profile()` set `actionModeBatchFlash` thành checked **trước khi group tồn tại** — lúc group được dựng ngay sau đó, cả 2 action đều đã checked=true sẵn (`actionModeFlash` mặc định checked="true" trong `.ui`), và `QActionGroup` chỉ enforce exclusivity khi có 1 action **đổi** trạng thái check, không tự sửa lại trạng thái đã checked sẵn lúc `addAction()`. Kết quả: cả 2 menu item cùng hiện checked, bấm cái nào cũng không đổi gì (đã checked sẵn nên không có signal `toggled` nào bắn ra) — app bị **kẹt vĩnh viễn ở Batch Flash mode**, không có cách nào quay lại Flash qua menu. Verify trực tiếp bằng file settings thật trên máy (`~/.config/tranph9/SFlash.ini` đang có sẵn `flash/mode=batch` từ lần user test Phase 4.88) — tái hiện đúng y hệt. Fix: ngay sau khi dựng `_mode_action_group`, ép lại `actionModeFlash.setChecked(not actionModeBatchFlash.isChecked())` — dùng trạng thái đã nạp đúng của `actionModeBatchFlash` (do `load_profile()` set) làm nguồn chân lý, sửa lại `actionModeFlash` cho khớp bây giờ khi group đã tồn tại.
- **`gui/main_window.ui`**: bỏ cột `"#"` khỏi `tableWidgetBatchLog` (5 cột → 4 cột: Serial Number, Timestamp, Result, Duration) — `stepsTable`/`segmentsTable` trong cùng file này vốn không có cột số thứ tự riêng, dựa hẳn vào vertical header mặc định của `QTableWidget` để đánh số dòng, nên đây là sửa cho khớp lại đúng convention sẵn có thay vì giữ 2 nguồn đánh số dòng trùng nhau. Regenerate `gui/ui_main_window.py` bằng `pyside6-uic`.
- **`gui/batch_flash.py`**: `_append_batch_log_row()` bỏ tham số `index` không còn dùng (cột "#" đã xoá) và dịch lại chỉ số cột cho 4 cột còn lại (Result giờ là cột 2, dùng để set tooltip lý do FAIL/ABORTED); `setup_batch_flash()` thêm vòng lặp set `QHeaderView.ResizeMode.Stretch` cho toàn bộ cột của `tableWidgetBatchLog` — theo đúng convention `setStretchLastSection(True)` mà `stepsTable`/`segmentsTable` đã dùng (`gui/flash_tab.py`), nhưng áp cho **mọi** cột thay vì chỉ cột cuối, vì bảng Batch Log không có cột nào đặc biệt rộng hơn cột khác (khác `traceTable`, nơi có 1 cột Data cần rộng hơn hẳn) — Stretch đều giúp 4 cột chia đều hết chiều rộng bảng, không còn khoảng trống bên phải.

### Đã kiểm tra

- Test mới trong `tests/test_gui_smoke.py::TestSettingsProfile`: mở rộng `test_batch_mode_persists_across_restart` thêm assertion `actionModeFlash.isChecked()` phải là `False` sau khi restore mode batch (test cũ không check dòng này nên không bắt được bug); thêm `test_restoring_batch_mode_does_not_wedge_switching_back_to_flash` — mô phỏng đúng kịch bản restart với mode đã lưu là batch, rồi bấm lại `actionModeFlash`, assert quay về Flash mode thành công (`flashButton.text() == "Flash"`).
...
### Phase 4.94: Parallel Flash Task 5-6 — Lambda Phá Vỡ Queued Connection Xuyên Luồng, Phát Hiện Lúc Viết Test Thật

Đang implement Task 5-6 của plan `docs/superpowers/plans/2026-09-06-parallel-flash.md` (mỗi panel có `TestConnectionWorker`+`FlashWorker` riêng, nối tiếp Identify → Flash) theo đúng code mẫu trong plan (dùng `lambda ..., p=panel: self._method(p, ...)` để truyền thêm tham số `panel` vào handler dùng chung cho cả 4 panel). Code chạy được về mặt logic (test `phase == "pass"` pass), nhưng test thật với `QThread` (`tests/test_parallel_flash_threading.py::TestPerPanelFlash`) timeout: `panel["flash_thread"].isRunning()` không bao giờ trả về `False` dù `FlashWorker` đã chạy xong thật sự — dẫn tới crash `QThread: Destroyed while thread is still running` lúc thoát process, đúng loại lỗi CLAUDE.md's "Threading model" đã cảnh báo.

Debug bằng script headless độc lập (in `isRunning()`/`isFinished()` mỗi 500ms) xác nhận: `flash_thread` không bao giờ dừng, dù `_on_panel_flash_finished()` đã chạy đúng (set `phase = "pass"`) — có nghĩa `flash_finished` signal đã emit và tới được 1 slot, nhưng KHÔNG tới được `thread.quit()`. Viết repro tối giản (2 `QObject` trong 1 `QThread`, so sánh nối signal tới bound-method thật vs. tới lambda) xác nhận nguyên nhân gốc: **PySide6 chỉ tự động nhận diện được kết nối "queued" xuyên luồng khi slot là bound-method thật của 1 `QObject`** (có `thread()` xác định) — nối tới 1 lambda (dù lambda đó gọi lại bound-method bên trong) khiến PySide6 không xác định được "receiver" thuộc luồng nào, nên fallback về kết nối "direct" (chạy đồng bộ ngay trong luồng đang emit), kể cả khi ép `Qt.ConnectionType.QueuedConnection` tường minh. Hệ quả: `_start_flash_for_panel()` (được gọi từ trong `_on_identify_finished_for_panel`, cũng nối qua lambda) chạy NGAY TRONG LUỒNG WORKER CỦA IDENTIFY thay vì luồng chính — `QThread()`/`FlashWorker()` mới bị tạo ra với "chủ sở hữu" là luồng Identify (đã dừng ngay sau đó), nên `flash_thread.quit()` sau này bị queue tới 1 event loop đã chết, không bao giờ được xử lý.

Thử sửa bằng `self.sender()` (đọc lại panel qua tham số ẩn trong bound-method dùng chung) gặp lỗi thứ 2: `self.sender()` trả về `None` — vì mỗi worker cũng tự nối `finished`/`flash_finished` tới `deleteLater()` của chính nó (kết nối same-thread, chạy trực tiếp/ngay lập tức trong luồng worker), nên tới lúc slot queued chạy được trên luồng chính thì worker gốc (sender) có thể đã bị Qt xoá thật sự — `sender()` trả về `None` đúng theo hành vi tài liệu hoá của Qt trong trường hợp này.

### Thay đổi

- **`gui/parallel_flash.py`**: thêm class mới `_PanelSignalRouter(QObject)` — 1 object nhỏ, sống suốt vòng đời panel (không bị `deleteLater()` xoá như worker), lưu `panel` làm thuộc tính thường (`self.panel`), cung cấp các bound-method thật (`on_identify_finished`, `on_ecu_info`, `on_progress_changed`, `on_step_started`, `on_log_message`, `on_flash_finished`, `on_flash_aborted`, `on_identify_thread_finished`, `on_flash_thread_finished`) làm đích nối cho MỌI signal xuyên luồng từ `identify_worker`/`flash_worker`/`identify_thread`/`flash_thread`. Mỗi panel có đúng 1 router riêng (`panel["router"]`, tạo 1 lần trong `_build_parallel_panel()`). Toàn bộ lambda dùng để truyền `panel` (trong `_start_identify_for_panel()` và `_start_flash_for_panel()`) bị xoá, thay bằng `router.on_xxx`. Các handler nội bộ (`_on_identify_finished_for_panel`, `_on_panel_flash_finished`, ...) giữ nguyên chữ ký `(self, panel, ...)` — router gọi lại chúng, không đổi logic nghiệp vụ.
- Thêm đoạn note dài trong module docstring giải thích rõ 2 quy tắc mới riêng cho file này (không dùng lambda cho signal xuyên luồng; không dùng `self.sender()` khi worker tự `deleteLater()` chính nó) để không ai vô tình tái phạm khi thêm panel/luồng mới sau này.
...
### Phase 4.96: Parallel Flash — Sửa Layout/Màu Sắc Cho Đúng Với Mockup Đã Duyệt

Sau khi merge Parallel Flash, user mở app thật, chụp 2 screenshot so sánh: 1 ảnh của chính bản mockup HTML đã duyệt lúc brainstorm (combo + nút "View Log" nằm cùng hàng, nút Flash/Abort màu + progress bar nằm cùng 1 hàng, "Start All" xanh dương/"Abort All" đỏ), và 1 ảnh chụp app thật đang chạy (`main.py`) — mỗi panel chỉ là 1 cột dọc đơn giản: combo (không có "View Log"), "SN: —", nút "Flash" (trắng, không màu) chiếm cả hàng, progress bar (trắng, không màu) chiếm cả hàng, dòng trạng thái. Hỏi tại sao layout không giống mockup.

Đọc lại đúng file mockup đã lưu (`docs/superpowers/specs/2026-09-06-parallel-flash-design.md` kèm bản HTML tương tác) xác nhận: Task 3 của plan chỉ dựng đúng cấu trúc CHỨC NĂNG (đủ widget, đủ hành vi) nhưng bỏ qua hoàn toàn phần thiết kế trực quan đã duyệt — thiếu hẳn nút "View Log", xếp Flash/progress bar thành 2 hàng riêng thay vì chung 1 hàng, và không có màu sắc động theo trạng thái nào cả (Flash lúc nào cũng trắng, Abort cũng trắng, progress bar lúc nào cũng trắng). Đây không phải bug kỹ thuật mà là khoảng trống giữa bản thiết kế đã duyệt và phần code thực thi — thuộc nhóm lỗi "khớp lại với thiết kế đã chốt", cùng loại với các lần sửa UI theo screenshot khác trong phiên này.

### Thay đổi

- **`config/settings.py`**: thêm bộ hằng số màu mới (`ACCENT_COLOR`/`_DARK`, `DANGER_COLOR`/`_DARK`, `SUCCESS_COLOR`/`_DARK` cùng các biến `_HOVER_COLOR`, và `DISABLED_BUTTON_*`) — lấy chính xác từ hex đã dùng cho `#flashButton`/`#buttonStopBatch`/`#buttonExportBatchReport` trong `resources/style.qss`/`style_dark.qss`, dùng để tô màu động cho widget dựng runtime (không thể dùng rule QSS tĩnh theo object name vì object đó không có ID duy nhất và màu phải đổi theo trạng thái).
- **`gui/parallel_flash.py`**: `_build_parallel_panel()` viết lại layout — thêm nút `view_log_button` ("View Log", chuyển `tabWidgetParallelDetail` sang đúng tab của panel khi bấm), ghép `combo`+`view_log_button` vào 1 `QHBoxLayout`, ghép `flash_button`+`progress_bar` vào 1 `QHBoxLayout` khác (khớp đúng bản mockup) thay vì xếp lần lượt theo cột dọc. Thêm `_apply_panel_button_style(panel, kind)`/`_apply_panel_progress_style(panel, kind)` (kind ∈ {"accent","danger"} cho nút, {None,"success","danger"} cho progress bar) áp `setStyleSheet()` trực tiếp lên từng widget theo state hiện tại của panel, lưu lại `kind` đã áp (`_button_kind`/`_progress_kind`) để `_recolor_parallel_panels()` (gọi từ `action_toggle_dark_mode()`) tô lại đúng theme mới — cùng pattern "lưu kind, tô lại lúc đổi theme" đã dùng cho `stepsTable`/`segmentsTable`/Batch Log ở Phase 4.90-4.91, tránh lặp lại đúng lỗi "màu bị kẹt theo theme lúc tô" đã gặp trước đó.
- **`resources/style.qss`/`style_dark.qss`**: thêm rule tĩnh cho `#buttonParallelStartAll` (xanh dương, như `#flashButton`), `#buttonParallelAbortAll` (đỏ, như `#buttonStopBatch`), `#buttonParallelViewLog` (nút pill viền mảnh, trong suốt) — cả 2 file, đủ base/hover/disabled.
- **`gui/menu_bar.py`**: `action_toggle_dark_mode()` gọi thêm `self._recolor_parallel_panels()`.
- Sửa lại 1 hằng số magic-number khác bị lệch tiếp: gộp 5 hàng/panel xuống còn 4 hàng làm giảm chiều cao tối thiểu thật của tab Parallel Flash sau khi kích hoạt layout đầy đủ (`test_resize_after_full_screen_exits_full_screen_first`) — quay lại đúng `789` (giá trị trước khi Parallel Flash được thêm ở Phase 4.92), không còn là `800` nữa.

### Đã kiểm tra

- Verify bằng script headless `.grab()` (áp đúng `app.setStyleSheet(load_stylesheet(...))` như `main.py` thật làm — lúc đầu quên bước này khiến ảnh chụp không lên màu, tưởng nhầm là bug, tự phát hiện khi so sánh với `tests/qt_test_utils.py`'s `get_app()` không tự áp stylesheet) ở cả Light và Dark Mode — xác nhận layout/màu khớp đúng bản mockup: PASS xanh lá 100%, FAIL/ABORTED đỏ 55%, đang chạy nút Abort đỏ + progress bar xanh dương chạy, panel "Not Selected" nút Flash xám mờ đúng trạng thái disabled.
- Test mới: `TestParallelFlashPanelScaffolding` thêm 5 test (View Log chuyển tab, nút khởi tạo màu accent, nút chuyển danger lúc chạy, progress bar chuyển success/danger lúc pass/fail), `TestParallelPanelRecolorsOnThemeToggle` (2 test, đổi theme lúc đang chạy/lúc đã pass), `tests/test_style.py::test_both_themes_style_parallel_flash_buttons`. 2 test đầu tiên tự viết bị lỗi để lộ 1 thread thật chạy nền không dọn — sửa bằng cách gọi `_abort_panel()` cuối test thay vì để mặc kệ.
- Full suite: 448 test pass (skipped=2, không đổi). `tests/test_flash_threading.py` (9 test) pass riêng.

### Phase 4.97: Thêm Nút "Settings" Cho Từng Channel — Basic Communication Per-Panel

User yêu cầu thêm 1 nút "Settings" (cạnh "View Log") cho từng channel của Parallel Flash, bấm vào hiện popup cho phép chỉnh Request/Response/Functional Request CAN ID riêng cho channel đó. Đây là tính năng MỚI (không phải bug fix), và xung đột trực tiếp với 1 quyết định thiết kế đã chốt lúc brainstorm Parallel Flash ban đầu: "Same firmware and same CAN protocol settings ... for all 4 panels — only the physical hardware channel differs per panel" — nên áp dụng đúng quy tắc CLAUDE.md "brainstorm fit trước khi implement feature mới": dừng lại, kiểm tra code thật (`get_can_config()`, `FlashWorker`/`TestConnectionWorker`) trước khi trả lời, phát hiện thêm rằng "Functional Request CAN ID" **chưa hề được config hoá ở bất kỳ đâu** trong app (hardcode `0x700` thẳng trong `FlashWorker.__init__()`) — tức là scope thực tế lớn hơn ban đầu tưởng (không chỉ "cho override 1 setting đã có sẵn theo từng panel", mà còn "biến 1 hằng số ẩn thành 1 giá trị config được lần đầu tiên"). Hỏi user 1 câu hỏi (AskUserQuestion) về mô hình override: mặc định = giá trị global, sửa xong thì tách riêng hẳn cho channel đó (không đồng bộ tiếp theo Configure tab sau này) — user chọn phương án này.

User yêu cầu xem mockup HTML trước khi implement — làm 1 mockup mới (`parallel_channel_settings_mockup.html`), tái dùng đúng token màu/layout của mockup Parallel Flash gốc đã duyệt, thêm nút "Settings" pill cạnh "View Log", popup modal có 3 field hex + nút "Reset to shared"/Cancel/Save, và 1 chỉ báo trực quan (viền + nền tô sáng) trên nút Settings khi channel đó đã có giá trị riêng. User duyệt mockup, yêu cầu implement.

### Thay đổi

- **`core/flash_controller.py`**: `FlashWorker.__init__()` thêm tham số `functional_id=0x700` (thay hằng số cứng trước đó) — cùng pattern additive/backward-compatible như `security_lock` ở Task 4 của Parallel Flash.
- **`core/test_connection.py`**: `TestConnectionWorker.__init__()` thêm tham số tương tự, truyền xuống `FlashWorker` nội bộ mà `run()` tự dựng để tái dùng `_setup_uds_client()`.
- **`gui/parallel_channel_settings_dialog.py`** (mới): `ParallelChannelSettingsDialog` — dialog form thuần (không `QThread`/worker, khác `TestConnectionDialog`/`GitLabFetchDialog`) với 3 `QLineEdit` hex, validate lúc bấm Save (sai hex thì hiện lỗi đỏ, không đóng dialog), nút "Reset to shared" set 1 cờ riêng để caller biết cần xoá override. Logic validate/parse nằm ở method gọi được trực tiếp (`_on_save_clicked()`), không bắt buộc phải `exec()` dialog thật mới test được — cùng lý do CLAUDE.md đã ghi cho `_write_log_file()`.
- **`gui/parallel_flash.py`**: thêm `settings_button` (nút "Settings", cùng hàng với combo+View Log) và `comm_settings` (None hoặc dict `{tx_id,rx_id,functional_id}`) vào panel dict. `_open_parallel_channel_settings()` mở dialog (prefill = override hiện có, hoặc giá trị global + `0x700` cho functional), lưu/xoá `comm_settings` theo lựa chọn Save/Reset. `_resolve_panel_comm_ids(panel, can_config)` là điểm DUY NHẤT quyết định panel dùng giá trị nào — gọi từ cả `_start_identify_for_panel()` lẫn `_start_flash_for_panel()` khi dựng `TestConnectionWorker`/`FlashWorker`. `_apply_settings_button_style()` tô nút Settings (viền+nền accent) khi có override, tái dùng đúng pattern lưu-kind-tô-lại-lúc-đổi-theme đã có cho Flash/Abort button và progress bar. Settings button bị disable trong lúc panel đang identifying/flashing, giống combo.
- **`gui/settings_profile.py`**: `save_profile()`/`load_profile()` thêm lưu/khôi phục `comm_settings` mỗi panel (`parallel/panel{i}/commCustomized|commTxId|commRxId|commFunctionalId`) — độc lập với việc channel có được chọn hay không (khác với channel selection, vốn chỉ lưu khi `selected=True`).
- **`config/settings.py`**: thêm `HIGHLIGHT_BG_COLOR`/`_DARK` (tái dùng đúng màu "active/selected" đã có sẵn cho `QTabBar::tab:selected` trong `resources/style.qss`/`style_dark.qss`, không bịa màu mới).
- **`resources/style.qss`/`style_dark.qss`**: thêm rule tĩnh `#buttonParallelChannelSettings` (pill mặc định, giống `#buttonParallelViewLog`) — trạng thái "đã customize" được tô động qua Python (`setStyleSheet()`), không qua QSS tĩnh, vì phụ thuộc state runtime.

### Đã kiểm tra

- Test mới xác nhận `functional_id` custom thật sự chảy tới tận trace log (`FuncGroup-0x710` thay vì `0x700`) cho cả `FlashWorker` (`tests/test_flash_controller.py`) lẫn `TestConnectionWorker` (`tests/test_test_connection.py`) — không chỉ lưu ở constructor rồi không dùng.
- `tests/test_parallel_channel_settings_dialog.py` (5 test): prefill đúng giá trị, validate hex đúng/sai, Save/Reset trả kết quả đúng — không cần `exec()` dialog thật.
- `TestParallelChannelSettingsButton` (7 test, `tests/test_gui_smoke.py`): nút mở dialog với đúng giá trị mặc định/đã customize, Save/Reset/Cancel cập nhật đúng `comm_settings` + màu nút, disable đúng lúc đang chạy. 1 lỗi tự viết bị lộ: test giả định mặc định `tx_id=0x778` — sai, vì `MainWindow` mới dựng đã áp sẵn profile mặc định (radar side/Suzuki) với `tx_id=0x77B` thật — sửa test đọc giá trị thật qua `get_can_config()` thay vì hardcode.
- `TestPerPanelIdentify`/`TestPerPanelFlash` (`tests/test_parallel_flash_threading.py`) thêm 2 test xác nhận `identify_worker`/`flash_worker` thật sự nhận đúng `tx_id`/`rx_id`/`functional_id` đã customize của panel, không phải giá trị global.
- `TestParallelPanelChannelPersistence` thêm 2 test xác nhận `comm_settings` sống sót qua restart, và panel chưa customize vẫn `None` sau restart.
- Verify headless `.grab()` (Light Mode): nút Settings của channel đã customize hiện đúng viền+nền accent, dialog thật hiện đúng 3 field pre-fill + note + 3 nút.
- Full suite: 467 test pass (skipped=2, không đổi). `tests/test_flash_threading.py` (9 test) pass riêng.

### Phase 4.98: Bỏ 4 Tab Log Riêng, Dùng Chung Tab Information/Trace Có Sẵn

User ban đầu yêu cầu đổi nút "View Log" thành "View Trace" và cho mỗi tab channel hiện log dạng bảng Trace (giống tab Trace chung của app) thay vì text thuần — nhưng user tự dừng lại (interrupt) giữa lúc đang tìm hiểu, rồi đổi hẳn sang 1 đề xuất khác đơn giản hơn: bỏ hẳn khu vực 4 tab riêng cho từng channel (`tabWidgetParallelDetail`), thay vào đó khi bấm "View Log" ở 1 channel thì đúng 2 tab **Information**/**Trace** đã có sẵn của app (dùng chung cho Single Flash/Batch Flash) sẽ tự chuyển sang hiện log của channel đó. Kiểm tra `main_window.ui` xác nhận đề xuất này khả thi và thật ra gọn hơn: `outputTabWidget` (Information+Trace) vốn đã là **sibling** của `tabWidget` (Single Flash/Parallel Flash/Configure) ngay trong layout gốc (`verticalLayout_root`, stretch `3,2`) — tức đã hiển thị sẵn bên dưới bất kể đang ở tab nào, kể cả Parallel Flash — nên 4 tab riêng trước đó là dư thừa.

Vì Information/Trace là 2 widget DÙNG CHUNG cho cả app (Single Flash, Batch Flash, và giờ cả 4 channel Parallel Flash), trong khi 4 channel có thể đang chạy ĐỒNG THỜI — cần thêm khái niệm "channel đang được xem" (`_parallel_active_panel_index`) và mỗi panel tự giữ buffer riêng (`info_lines`, `trace_entries`) để không mất dữ liệu của các channel không đang xem. Message mới của 1 channel: luôn được lưu vào buffer riêng của channel đó; CHỈ ghi thêm trực tiếp lên 2 tab chung nếu channel đó đang là channel active. Bấm "View Log" xoá sạch 2 tab chung rồi phát lại (replay) đúng lịch sử đã buffer của channel đó — dùng đúng timestamp đã lưu lúc sự kiện xảy ra thật (không phải giờ hiện tại lúc replay).

### Thay đổi

- **`gui/main_window.ui`**: xoá hẳn `tabWidgetParallelDetail` và 4 `textEditParallelChannel{N}Log` — Parallel Flash tab giờ kết thúc ngay sau lưới 4 channel. Regenerate `gui/ui_main_window.py`.
- **`gui/main_window.py`**: tách `_append_information_line(formatted_line)` ra khỏi `log_information()` — nhận 1 dòng ĐÃ có timestamp sẵn, dùng cho cả live-update lẫn replay lịch sử cũ (nếu gọi thẳng `log_information()` cho replay, mỗi dòng cũ sẽ bị đóng dấu giờ HIỆN TẠI thay vì giờ nó thực sự xảy ra — sai). `log_trace_row()`/`_add_trace_row()` giữ nguyên, tái dùng trực tiếp cho cả 2 trường hợp vì `trace_row` dict đã tự mang timestamp thật từ lúc `FlashWorker` phát ra.
- **`gui/parallel_flash.py`**: `setup_parallel_flash()` thêm `self._parallel_active_panel_index = None` (không channel nào tự chiếm 2 tab chung chỉ vì tab Parallel Flash được mở — chỉ đổi khi bấm "View Log" thật). Panel dict bỏ `log_widget`, thêm `info_lines`/`trace_entries` (buffer riêng). `_view_parallel_panel_log()` viết lại hoàn toàn: set active index, xoá + phát lại 2 tab chung từ buffer của channel đó. `_log_parallel_panel()` (info), `_on_panel_trace_message()` (dòng trace dạng SYSTEM), `_on_panel_trace_row()` (dòng trace có cấu trúc, MỚI — trước đây `trace_row` của `FlashWorker` chưa hề được nối cho Parallel Flash) — cả 3 đều buffer trước, chỉ ghi lên 2 tab chung nếu đang active. `_PanelSignalRouter` tách `on_log_message` (gộp cả info lẫn trace trước đây) thành `on_information_message`/`on_trace_message`/`on_trace_row` riêng — khớp đúng quy ước router-là-bound-method-thật đã ghi trong docstring file này (Phase 4.94).

### Đã kiểm tra

- Script headless thật (không phải test giả lập): nạp firmware thật, chọn Virtual ECU cho 2 channel, xem live channel 1 trong lúc CẢ 2 channel cùng flash — xác nhận bảng Trace chung tăng dần đúng 35 dòng khớp 1:1 với buffer riêng của channel 1 trong suốt quá trình (live update đúng), channel 2 vẫn tự buffer đủ 35 dòng ở hậu trường dù không hiện gì — chuyển sang xem channel 2 thì bảng Trace chung đổi đúng thành 35 dòng của channel 2.
- Test mới (`tests/test_gui_smoke.py`, `TestParallelFlashPanelScaffolding`): bấm View Log đổi đúng active index; phát lại đúng buffer (cả dòng SYSTEM lẫn dòng có cấu trúc); message của channel KHÔNG active không hiện lên 2 tab chung (chỉ vào buffer riêng); message của channel ĐANG active hiện ngay lập tức; chuyển qua lại 2 channel chỉ hiện đúng lịch sử riêng của channel đang xem, không lẫn lộn.
- Xoá `test_four_detail_log_tabs_exist` (widget không còn tồn tại); viết lại `test_view_log_button_switches_the_shared_detail_tab` cũ thành các test theo hành vi mới.
