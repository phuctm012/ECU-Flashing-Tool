...
- **`gui/menu_bar.py`**: `MainWindow.open_gitlab_fetch_dialog()` là handler dùng chung — cả `actionLoadFromGitLab.triggered` lẫn `buttonLoadFromGitLab.clicked` cùng nối vào, đúng pattern "1 handler, 2 entry point" đã dùng cho `open_test_connection_dialog()`.
...
- 3 bug thật phát hiện qua review/implement, đều sửa trước khi merge:
  ...
  - Task 3: thêm `buttonLoadFromGitLab` làm minimum height thật (sau khi show) của `MainWindow` tăng thêm 31px, âm thầm phá preset resize "Medium (1366 × 768)" (Qt tự kẹp về 1366×779). Sửa bằng cách cập nhật preset lên 1366×789 (kích thước thật đã verify đạt được) đồng bộ ở `gui/main_window.ui`, `gui/menu_bar.py` và 2 test liên quan.
