...
### Phase 4.89: Fix Text Nút Batch Flash Bị Cắt Chữ + Bug Kẹt Mode Sau Khi Restore Từ Settings

User chạy thử tính năng Batch Flash (Phase 4.88) trên app thật, gửi screenshot: nút chính hiển thị "tart Batc" thay vì "Start Batch" — chữ bị cắt vì nút quá hẹp so với text dài. User yêu cầu rút gọn, "chỉ cần hiển thị Start/Next là đủ rồi".
...
- Full suite: 416 test pass (415 cũ + 1 test mới); `tests/test_flash_threading.py` (9 test) chạy riêng không bị ảnh hưởng dù không đụng gì tới `flash_button_clicked()`. App headless khởi động/đóng sạch không lỗi.
