...
Danh sách chia làm 2 phần: **Chưa Hoàn Thành** trước, **Đã Hoàn Thành** sau — đánh số
liên tục từ 1 tới hết, không theo thứ tự thời gian implement nữa (xem "Ghi chú" cuối
file để biết bối cảnh/lịch sử theo từng đợt trước khi đánh số lại).

## Chưa Hoàn Thành

### 1. Cột "Signature" trong bảng Datablocks — luôn rỗng
...
### 4. Đọc / Xoá DTC Sau Flash

- **Trạng thái**: ⬜ Chưa implement (đề xuất tính năng mới, không phải bug)
- **Vấn đề**: `README.md`'s bảng "Trạng Thái Phát Triển" đã ghi mục này là 🔜 từ trước. UDS Client hiện có Communication Control/DTC Setting (bật/tắt DTC trong lúc flash — xem `uds_client.py`) nhưng chưa có màn hình đọc danh sách DTC hiện có trên ECU hay xoá riêng (ClearDiagnosticInformation, SID `0x14`) — không có cách xác nhận ECU "sạch lỗi" sau khi flash xong ngoài việc tự đoán.
- **Đề xuất**: thêm 1 action mới (menu Tools hoặc nút riêng trong tab Flash sau khi hoàn tất) gọi `ReadDTCInformation` (SID `0x19`) hiển thị danh sách DTC (mã lỗi + status byte), kèm nút "Clear DTCs" gọi `ClearDiagnosticInformation` (SID `0x14`). Có thể tận dụng đúng dialog pattern đã có ở `TestConnectionDialog` (chạy qua `QThread` riêng, không block GUI).
- **Vị trí liên quan**: `communication/uds_client.py` (thêm service mới), `gui/` (dialog/action mới), `core/` (nếu cần 1 worker riêng).

### 5. Chế Độ Flash Hàng Loạt (Batch / Production Mode)

- **Trạng thái**: ⬜ Chưa implement (đề xuất tính năng mới, không phải bug)
- **Vấn đề**: Tool hiện chỉ hỗ trợ flash **1 ECU tại 1 thời điểm** — mỗi lần xong phải tự nạp lại/bấm Flash lại thủ công cho ECU tiếp theo. Nếu dùng ở dây chuyền/số lượng lớn (nhiều ECU cùng firmware), quy trình lặp lại thủ công tốn thời gian và dễ bỏ sót không theo dõi được PASS/FAIL tổng thể.
- **Đề xuất**: 1 chế độ/tab riêng — giữ nguyên firmware + cấu hình đã chọn, cho phép bấm "Flash Next" lặp lại liên tục (hoặc tự động chờ phát hiện ECU mới trên bus), đếm số lượng PASS/FAIL, log serial number/VIN đọc được qua DID cho từng lần — cần thiết kế kỹ hơn (spike/brainstorm riêng) trước khi viết plan, vì đụng tới luồng chính `core/flash_controller.py` và UX mới hoàn toàn, không phải thay đổi nhỏ.
- **Vị trí liên quan**: `core/flash_controller.py`, `gui/flash_tab.py` (hoặc tab mới), cần bàn thêm trước khi thiết kế chi tiết.

### 6. Kiểm Tra Firmware Trước Khi Flash (Đối Chiếu Với ECU)

- **Trạng thái**: ⬜ Chưa implement (đề xuất tính năng mới, không phải bug)
- **Vấn đề**: Hiện tại không có bước nào đối chiếu file firmware đã chọn với chính ECU đang kết nối trước khi flash — nếu chọn nhầm file (sai part number/phiên bản), tool vẫn flash bình thường, chỉ phát hiện được sau khi xong (hoặc không phát hiện được).
- **Đề xuất**: trước khi cho phép bấm Flash (hoặc như 1 bước đầu trong flash sequence), đọc part number/phiên bản hiện tại của ECU qua `ReadDataByIdentifier` (đã có sẵn cơ chế ReadDID), so sánh với thông tin kỳ vọng của firmware đã chọn (cần định nghĩa thêm nguồn "expected part number" — có thể từ tên file, hoặc 1 file mapping riêng) — cảnh báo rõ ràng nếu không khớp thay vì cho flash luôn. Cần bàn kỹ hơn về nguồn dữ liệu "expected" trước khi thiết kế.
- **Vị trí liên quan**: `communication/uds_client.py` (ReadDID đã có), `core/flash_controller.py` hoặc `gui/flash_tab.py` (bước kiểm tra mới trước khi cho Flash chạy).

## Đã Hoàn Thành

### 7. Combo "Checksum Method" — hoàn toàn chết (dead)
...
### 8. Checkbox cột đầu bảng Datablocks — không có tác dụng
...
### 9. Segments table hiện dữ liệu demo giả khi chưa nạp file nào
...
### 10. Lưu/Nạp Lại Cấu Hình (Profile)
...
### 11. Xuất Báo Cáo Phiên Flash (PDF/HTML)
...
### 12. Tổng Kết PASS/FAIL Rõ Ràng Sau Verify Memory

- **Trạng thái**: ✅ Đã xử lý (2026-08-23)
- **Đã làm**: `core/flash_sequence.py` — thêm `"action": "verify"` vào `params` của step "Verify Memory" (cả `DEFAULT_FLASH_SEQUENCE` lẫn `SUZUKI_SLP1_FLASH_SEQUENCE`), cùng cách `"action": "erase"` đã dùng cho Erase Memory. `core/flash_controller.py`'s `_execute_routine()` bọc `routine_control()` trong `try/except` — thành công thì emit `"✓ Verify Memory: PASS"` khi `action == "verify"` (thay vì message chung chung `"Routine 0xFF01 completed"`); lỗi (NRC) thì emit `"✗ Verify Memory: FAILED"` **trước khi re-raise** — cơ chế abort hiện có (`_execute_step()`'s catch-all, đã emit `"Error: {e}"`) vẫn chạy y hệt như cũ, chỉ thêm 1 dòng rõ ràng hơn phía trên nó. Không cần UDS service mới, không cần đổi GUI/Export Report — cả 2 đều đọc `information_message`/`informationText` như mọi message khác nên tự động hiện ra.
- **Vấn đề (trước khi xử lý)**: `_execute_routine()` — bước "Verify Memory" chỉ gọi `RoutineControl` (0x31) và nếu ECU không trả NRC thì coi là xong, emit đúng 1 dòng chung chung `"Routine 0xFF01 completed"` — không có dòng kết luận rõ ràng kiểu "Verify: PASS"/"FAIL" như vFlash luôn hiển thị sau bước verify.
- **Vị trí liên quan**: `core/flash_sequence.py` (2 step "Verify Memory"), `core/flash_controller.py` (`_execute_routine()`).

### 13. Chưa Có Theme/QSS Nhất Quán Toàn App
...
### 14. Không Có Phản Hồi Hover/Pressed Trên Button

- **Trạng thái**: ✅ Đã xử lý (2026-08-23) — bundled vào cùng `resources/style.qss` ở mục #13, không cần file/logic riêng
...
- **Vị trí liên quan**: `resources/style.qss` (mục #13).

### 15. Icon App Mới Chưa Được Set Làm Window Icon Lúc Chạy
...
### 16. Progress Bar Nhảy Cứng Theo Từng Bước, Không Mượt
...
### 17. Màu Trạng Thái (Steps/Segments Table) Lệch Tông So Với Brand Color Của App
...
### 19. Bảng Steps/Segments/Trace Trống Trơn Lúc Chưa Chạy Flash — Thiếu Placeholder
...
### 21. Menu Edit — Clear Information Log / Clear Trace Table
...
### 22. Save / Load Project

- **Trạng thái**: ✅ Đã xử lý (2026-08-23)
- **Đã làm**: Module mới `gui/project_file.py` (`ProjectFileMixin`) — **File → Save Project As...** ghi file JSON đuôi `.ffproj` (tự thêm đuôi nếu user không gõ), gồm: đường dẫn + trạng thái tick/untick từng firmware đã nạp (đọc qua `datablock.file_path` có sẵn, không nhúng nguyên file), Hardware (virtual/channel), Radar Side index, Logical Link index, Security DLL path, Flash Sequence index. **File → Open Project...** đọc lại JSON, **thay thế hoàn toàn** danh sách datablock đang có (không merge) rồi nạp lại từng file qua `_load_firmware_file()` (hàm dùng chung đã tách ở mục #20 cho Recent Files) — file bị xoá/di chuyển hiện đúng dialog "Parse Error" có sẵn, không cần code thêm. Khôi phục các combo theo đúng pattern `load_profile()` đã có (tìm `itemData` khớp cho Hardware, guard `0 <= index < count()` cho các combo còn lại, kiểm tra `os.path.isfile()` trước khi khôi phục Security DLL).
- **Vấn đề (trước khi xử lý)**: `README.md`'s bảng "Trạng Thái Phát Triển" ghi mục này là 🔜 với đuôi file cũ `.vfp` (từ thời tool còn tên "VectorFlash"). Khác hẳn Profile hiện tại (`gui/settings_profile.py`) — Profile tự động lưu 1 file `.ini` duy nhất, ghi đè im lặng, không cho đặt tên/chọn nơi lưu, và không lưu danh sách firmware đã nạp.
- **Vị trí liên quan**: `gui/project_file.py` (mới), `gui/main_window.ui` (action `actionSaveProjectAs`/`actionOpenProject`), `gui/menu_bar.py` (wiring), `gui/main_window.py` (thêm `ProjectFileMixin`).

### 23. File → Close Window, Tools → Flash / Abort

- **Trạng thái**: ✅ Đã xử lý (2026-08-23)
- **Đã làm**: Menu **File → Close Window** (giữa "Open Project..." và "Exit") — cùng gọi `self.close()` như "Exit" (app chỉ có 1 cửa sổ duy nhất nên 2 action này tương đương về hành vi, tách riêng theo đúng yêu cầu). Menu **Tools → Flash** / **Abort** (trên cùng, tách biệt bằng separator với Test Connection/Export Report) — cả 2 đều gọi thẳng `self.flash_button_clicked()`, **tái dùng y hệt** logic toggle đã có của nút Flash/Abort trên tab Flash, không viết logic mới/không đụng vào phần code nhạy cảm với race condition `QThread` (xem "Threading model" trong `CLAUDE.md`). Để tránh trạng thái sai nhãn (bấm "Flash" từ menu lúc đang chạy sẽ vô tình gọi nhánh Abort, và ngược lại), thêm `_sync_flash_abort_menu_state()` — chỉ đọc `self.thread.isRunning()` (không đụng/sửa `self.thread`), bật đúng 1 trong 2 action tuỳ trạng thái hiện tại, gọi lại mỗi khi `menuTools.aboutToShow` (mở menu Tools) và 1 lần lúc `setup_menu_bar()`.
- **Vấn đề (trước khi xử lý)**: chưa có cách bắt đầu/dừng flash nào ngoài đúng 1 nút Flash/Abort trên tab Flash; chưa có "Close Window" riêng trong menu File.
- **Vị trí liên quan**: `gui/main_window.ui` (4 action mới: `actionCloseWindow`, `actionFlash`, `actionAbort`), `gui/menu_bar.py` (wiring + `_sync_flash_abort_menu_state()`).

### 24. View → Resize Window

- **Trạng thái**: ✅ Đã xử lý (2026-08-23)
- **Đã làm**: Submenu **View → Resize Window** — 3 kích thước cố định **Default (1100 × 850)** (khớp geometry mặc định trong `.ui`), **Medium (1366 × 768)**, **Large (1920 × 1080)** (kèm số kích thước ngay trong text menu cho rõ), cộng **Maximize Window**/**Full Screen**. `_resize_window(w, h)` (helper dùng chung cho 3 size cố định) tự gọi `showNormal()` trước nếu cửa sổ đang maximize/full screen — `resize()` trong Qt không có tác dụng khi đang ở 1 trong 2 trạng thái đó (window manager giữ nguyên maximize/full screen), nên nếu thiếu bước này, chọn 1 size cố định ngay sau khi đã Maximize/Full Screen sẽ im lặng không có tác dụng gì.
- **Vấn đề (trước khi xử lý)**: chưa có cách đổi kích thước cửa sổ nhanh nào ngoài kéo tay viền cửa sổ.
- **Vị trí liên quan**: `gui/main_window.ui` (submenu `menuResizeWindow` + 5 action), `gui/menu_bar.py` (`_resize_window()` + 5 handler).

### 25. Help → Export Issue... (bundle log/trace/config để debug)

- **Trạng thái**: ✅ Đã xử lý (2026-08-23)
- **Đã làm**: Module mới `gui/issue_export.py` (`IssueExportMixin`) — menu **Help → Export Issue...** xuất 1 file `.txt` thuần (không HTML) gồm: Environment (OS/Python version — thông tin mới, chưa từng có ở Export Report), Configuration (Hardware/Radar Side/Logical Link/Flash Sequence/Security DLL/Result), CAN Communication Details (đọc trực tiếp từ `tableWidgetCommDetails` — kể cả nếu user đã sửa tay), Loaded Datablocks (tick/untick + checksum), Information Log (đầy đủ), Trace (đầy đủ, dạng `A | B | C` mỗi dòng 1 frame). **Cố tình bỏ qua** bảng Steps (Information Log đã narrate lại đúng các bước đó, thường còn chi tiết hơn) và `tableWidgetCustomConfig` (mục #3 — chưa thực sự ảnh hưởng tới flash, đưa vào dễ gây hiểu lầm) để tránh trùng lặp — đúng yêu cầu "thông tin trùng nhau thì bỏ qua, tuỳ hướng debug".
- **Vấn đề (trước khi xử lý)**: muốn debug từ xa (user test trên máy Windows khác) phải tự gom nhiều nguồn riêng lẻ (Save Log .txt, Save Trace .csv, chụp màn hình Configure) rồi gửi nhiều file — chưa có cách xuất 1 file duy nhất đủ thông tin để debug. File xuất ra dùng đuôi `.txt` (không phải `.log`) — bundle này là 1 bản snapshot tổng hợp có cấu trúc tại 1 thời điểm, không phải log chạy liên tục kiểu server ghi đè dần, nên `.txt` khớp ngữ nghĩa hơn và mở được ngay trên mọi OS không cần app liên kết riêng.
- **Bổ sung (2026-08-23, theo yêu cầu user)**: thêm tuỳ chọn **đính kèm luôn file firmware đang nạp** — hữu ích khi nghi ngờ lỗi do sai định dạng firmware hoặc tool parse sai dữ liệu (metadata-only trong file `.txt` không đủ để tái hiện lỗi dạng này, cần file gốc). Cố tình làm **tuỳ chọn qua checkbox**, không tự động gộp vào mọi lần export — firmware ECU thật có thể là dữ liệu nhạy cảm/độc quyền, không nên tự động đính kèm mà không hỏi. Bấm **Export Issue...** giờ hiện 1 `QMessageBox` có `QCheckBox` "Include loaded firmware file(s) (.zip)" trước (mặc định **không tick**) — tick thì xuất `.zip` (gồm `issue.txt` + từng file firmware đang nạp, copy nguyên từ `datablock.file_path`, tự đổi tên nếu 2 file trùng tên tránh ghi đè trong zip), không tick thì vẫn xuất `.txt` như cũ. Dùng `zipfile` có sẵn trong Python, không cần cài thêm gì. File firmware không còn tồn tại trên đĩa (đã bị xoá/di chuyển từ lúc nạp) thì bỏ qua âm thầm, không lỗi.
- **Vị trí liên quan**: `gui/issue_export.py` (mới), `gui/main_window.ui` (action `actionExportIssue`), `gui/menu_bar.py` (wiring), `gui/main_window.py` (thêm `IssueExportMixin`).

## Ghi chú

**Bối cảnh/lịch sử theo từng đợt** (trước khi gộp lại thành 2 phần Chưa/Đã hoàn thành ở trên):

- Audit ban đầu (2026-08-22) thực hiện bằng cách liệt kê toàn bộ `name="..."` trong `gui/main_window.ui`, đối chiếu số lần xuất hiện trong `gui/*.py`/`main.py` qua `self.ui.<name>`/`self.<name>`. Các widget layout/label/container có 0 usage là bình thường (không cần wiring). Danh sách audit chỉ gồm các widget **tương tác được** (combo/checkbox/table có thể sửa) mà không có logic backend thật — không phải bug crash, app vẫn chạy ổn định, đây là các điểm UI "trông như hoạt động nhưng thực chất không" hoặc "chưa có tính năng thật đứng sau" (mục #1-3, #7-9 hiện tại — trong đó #7-9 đã xử lý, #1-3 còn tồn đọng).
- Mục #10-12 (Profile, Export Report, PASS/FAIL Verify Memory) là tính năng mới đề xuất thêm so với vFlash — mục tiêu sử dụng là thay thế vFlash để flash firmware ECU thật, nhưng vFlash (Vector) là công cụ thương mại rất rộng (ODX-F/PDX container, DoIP, multi-project database, licensing...) nên **parity 100% là phạm vi quá lớn**; các mục này chỉ là gap cụ thể được xác nhận cần thiết (2026-08-22, qua trao đổi trực tiếp) cho quy trình flash ECU thật hiện tại. Các ứng viên khác đã cân nhắc nhưng **chưa xác nhận cần** (chưa thêm vào danh sách): Operator ID/traceability theo người vận hành, theo dõi Programming Counter, đọc + so sánh SW/HW Version trước khi flash để cảnh báo downgrade, auto-detect ECU trên bus, hỗ trợ DoIP — hỏi lại nếu sau này thấy cần.
- Mục #13-19 (theme/QSS, hover/pressed, icon, progress bar, màu trạng thái, dark mode, placeholder) ghi lại từ audit UI ngày 2026-08-23, sau câu hỏi *"UI của tôi cần cải thiện, thêm animation, thay đổi màu sắc gì cho người dùng không?"*. Lúc đó app chỉ có vài label dùng `styleSheet` inline rời rạc, phần còn lại dùng nguyên theme mặc định Qt Fusion của OS — không nhất quán, không phản hồi hover/pressed, không animation. Nên làm theo thứ tự #13 trước (đòn bẩy lớn nhất — file `.qss` chung mà #14/#17 phụ thuộc vào), ưu tiên polish chuyên nghiệp (theme nhất quán, phản hồi tương tác, màu sắc hài hoà theo tông xanh của icon app) hơn animation kiểu app tiêu dùng — đây là công cụ kỹ thuật dùng trong môi trường sản xuất.
- Mục #20-21 (Recent Files, menu Edit) là 2 mục được chọn implement ngay khi user nhận xét menu bar (ban đầu, Phase 4.33, chỉ có File/Tools/Help với vài action cơ bản) còn ít chức năng và yêu cầu brainstorm — làm trực tiếp (bounded, không qua spec riêng).
- Mục #4-6 hiện còn tồn đọng (chưa implement) là kết quả 1 đợt brainstorm khác (2026-08-23) về hướng phát triển tiếp theo cho tool — 4 ứng viên được chọn ưu tiên lúc đó là Đọc/Xoá DTC, Save/Load Project, Batch Mode, Kiểm tra firmware trước khi flash; Save/Load Project đã hoàn thành ngay sau đó (nay là mục #22), 3 mục còn lại (#4-6) vẫn đang chờ.
- Mục #23-25 (Close Window/Flash/Abort, Resize Window, Export Issue) là các yêu cầu trực tiếp riêng lẻ từ user, mỗi mục 1 lần yêu cầu độc lập, không thuộc đợt brainstorm nào.
